...
export const ThreadItemBreadcrumbs = memo(({ id, nonInteractive }: ThreadItemBreadcrumbsProps) => {
  ...
  return <div>{JSON.stringify({ breadcrumbs, nonInteractive })}</div>

  // const allItems = useMemo(
  //   () =>
  //     breadcrumbs.map(({ id, title }) => (
  //       <BreadcrumbsItem key={id} url={'#'} isLink={!nonInteractive}>
  //         {title}
  //       </BreadcrumbsItem>
  //     )),
  ...
  // const [isShortened] = useState(false)
  // const [keptItemOnEachSide] = useState<number | undefined>()
